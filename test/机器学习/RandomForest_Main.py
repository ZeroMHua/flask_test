...
#         print("错误:变量%s不是定量变量，请重新选择" % var_c)
#     elif mdata[var_c].isna().any() == True:
#         print("错误:变量%s含有空值，请重新选择" % var_c)
# for var_d in vars_d:
#     if (type(mdata[var_d].iloc[0, 0]) not in ['str', 'bool', 'int']):
#         print("错误:变量%s不是定性变量, 请重新选择" % var_d)
#     elif mdata[var_d].isna().any() == True:
#         print("错误:变量%s含有空值，请重新选择" % var_c)
# if mdata[target].isna().any() == True:
#     print("错误:结局变量%s含有空值，请重新选择" % target)
# if len(set(mdata[target].iloc[:, 0])) > 2:
#     print("错误:结局变量类别数量大于2，目前只支持2分类的随机森林建模")
...
if type(mdata[target].iloc[0, 0]) in ['str', 'bool']:
    print(type(mdata[target].iloc[0, 0]))
...
x_train, x_test, y_train, y_test = train_test_split(mdata.drop(target[0], axis=1),
                                                    mdata[target[0]].astype(int), test_size=0.3)
...
"输出内容1：直接以字符串方式输出结果"
print("随机森林模型构建成功:模型准确度为%.4f,模型AUC评分为%.4f" % (pred_score, auc_score))
